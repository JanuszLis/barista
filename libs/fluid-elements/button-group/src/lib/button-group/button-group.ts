...
export class FluidButtonGroup extends LitElement {
  ...
  /**
   * Currently checked button-group-item id.
   * @attr
   * @type string || null
   */
  ...
  private _handleCheckedChange(
    ...
    const checkedButtonGroupItem = this._buttonGroupItems.find(
      (item) => item.checked,
    );
    if (checkedButtonGroupItem) {
      this.checkedId = checkedButtonGroupItem.id;
    }
  }
  ...
  private _handleKeyUp(event: KeyboardEvent): void {
    if (event.code === ARROW_RIGHT || event.code === ARROW_LEFT) {
      ...
      index = getNextIndex(index, this._buttonGroupItems.length, event.code);
  ...
  private _setFirstTabIndex(): void {
    ...
    const toSetTabindexItem = this._buttonGroupItems.find(
      (item) => !item.disabled,
    );
    if (toSetTabindexItem) {
      toSetTabindexItem.tabIndex = 0;
    }
  }
