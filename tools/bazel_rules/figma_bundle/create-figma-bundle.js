...
async function main() {
  const resolvedFiles = srcs
    ...
    .map((fileOrDirectory) => {
      ...
      if (typeof fileOrDirectory === 'string' && lstatSync(fileOrDirectory).isDirectory()) {
        return readdirSync(fileOrDirectory).map((fileName) => join(fileOrDirectory, fileName));
      } else {
  ...
  const inlineStyles = resolvedFiles.filter((file) => file.endsWith('.css'))
    .map((file) => readFileSync(file))
    .map((contents) => `<style>${contents}</style>\n`);
  const inlineScripts = resolvedFiles.filter((file) => file.endsWith('.js'))
    .map((file) => readFileSync(file))
