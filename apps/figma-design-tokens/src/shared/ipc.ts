...
export async function fetchTokens(): Promise<TokenStoreModel> {
  ...
  const reply = await getMessageFromWorker<FetchTokensResponseMessage>('fetch-tokens-response');
  return reply.data!;
}

/** Uploads style metadata with the worker process. */
export async function uploadStyleMetadata(styleMetadataCollection: StyleMetadataCollection): Promise<void> {
  postMessageToWorker({ type: 'upload-style-metadata-request', data: styleMetadataCollection });
  await getMessageFromWorker('upload-style-metadata-response', 'upload-style-metadata-userinterrupted');
}
...
export async function fetchStyleMetadata(): Promise<StyleMetadataCollection> {
  ...
  const reply = await getMessageFromWorker<FetchStyleMetadataResponseMessage>('fetch-style-metadata-response');
  return reply.data!;
}
