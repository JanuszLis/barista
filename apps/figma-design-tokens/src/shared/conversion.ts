...
const fontWeightToName = new Map<number, string[]>([
  ...
  [500, ['medium']],
  [600, ['demibold', 'semibold', 'demi-bold', 'semi-bold', 'demi bold', 'semi bold']],
  [700, ['bold']],
...
export function htmlToFigmaColor(htmlColor: string) : { r: number, g: number, b: number } {
  const colorWithoutHash = htmlColor.startsWith('#') ? htmlColor.slice(1) : htmlColor;
...
const centimetersToPixels = (centimeterValue: number) => centimeterValue * (96/2.54);
 ...
 */
export function cssToFigmaLineHeight(cssLineHeight: string): LineHeight | undefined {
...
export function getFirstAvailableFontFromFamily(
  ...
  const fontsInFamily = fontFamily.split(',').map(fontName => fontName.trim());

  for (const fontName of fontsInFamily) {
    // Try to find an exact match first
    const figmaFont = findFontByNameAndWeight(figmaFonts, fontName, cssFontWeightToNumber(fontWeight));
    if (figmaFont) {
      return figmaFont;
    }
    ...
    for (const fallbackFontName of fallbackFontNames) {
      const figmaFallbackFont = figmaFonts.find(font => font.fontName.family === fallbackFontName);
...
function findFontByNameAndWeight(figmaFonts: Font[], name: string, weight: number): Font | undefined {
  const familyFonts = figmaFonts.filter(font => font.fontName.family === name);
  const findFontWithWeight = (weight: number) : Font | undefined =>
    familyFonts.find(font => fontWeightToName.get(weight)?.includes(font.fontName.style.toLowerCase()));
  ...
  if (roundedWeight == 400) { // If the desired weight is 400, check 500 first
    font = findFontWithWeight(500);
  } else if (roundedWeight == 500) { // If the desired weight is 500, check 400 first
...
function cssSizeToUnitAndValue(cssSize: string): {value: number, unit: string} | undefined {
  if (!cssSize) {
    return undefined;
  }
