...
export function getCachedTokens(): TokenStoreModel | undefined {
  ...
  if (!themesJson) { // Figma returns an empty string if the key does not exist
    return undefined;
  }
...
export function getCachedRemoteStyles(): StyleMetadataCollection | undefined {
  ...
  if (!remoteStylesJson) { // Figma returns an empty string if the key does not exist
    return undefined;
  }
...
export function setLastRemoteStyleFetchTime(date: Date): void {
  figma.root.setPluginData('fluid-remote-styles-fetch-date', date.toISOString());
}
