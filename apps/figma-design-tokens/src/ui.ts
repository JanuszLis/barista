...
export function showMetadataUploadConfirmDialog(): Promise<boolean> {
  initUI();
  const confirmComponent = showUIComponent(
    'metadata-upload-confirmation');
  
  return new Promise((resolve) => {
    confirmComponent.addEventListener('confirm', () => resolve(true), { once: true });
    confirmComponent.addEventListener('cancel', () => resolve(false), { once: true });
  });
}
...
/** @internal Initializes the UI. Should be called before displaying components. */
function initUI(): void {
  // Setting a theme is mandatory to get correct colors
