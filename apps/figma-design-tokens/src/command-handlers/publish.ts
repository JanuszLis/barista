...
async function getLibraryStyles(): Promise<BaseStyle[]> {
  ...
  const publishStatuses = await Promise.all(allStyles.map(style => style.getPublishStatusAsync()));
  return allStyles; // .filter((_, index) => publishStatuses[index] !== 'UNPUBLISHED');
}
