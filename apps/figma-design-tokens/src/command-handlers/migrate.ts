...
export default async function run(): Promise<void> {
  for (const style of figma.getLocalPaintStyles()) {
    ...
    if (shade) { // The color is a "normal" palette color
      const newPaletteName = paletteMappings[paletteOrBaseColor] || paletteOrBaseColor.toLowerCase();
      style.name = `${newThemeName}/${newPaletteName}-${shade}`;
    } else { // The color is a base color
