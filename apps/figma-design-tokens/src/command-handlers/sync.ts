...
import { htmlToFigmaColor, getFirstAvailableFontFromFamily, cssSizeToPixels, cssToFigmaLineHeight, cssToFigmaTextTransform, designTokenToFigmaColorName } from '../shared/conversion';
...
async function synchronizeTokens(newTokens: TokenStoreModel): Promise<void> {
  ...
  if (deletedTokens) {
    const deletedTokenNames = Object.keys({ ...deletedTokens.palettes, ...deletedTokens.typography });
    ...
    if (deletedTokenNames.length > 0) {
      ...
      alert(`${deletedTokenNames.length} tokens were deleted:\n${deletedTokenNames.join(', ')} `
...
async function applyTypography(typographyTokens: DesignTokenMap): Promise<void> {
  ...
  for (const [name, typeSettings] of Object.entries(typographyTokens)) {
    ...
    const { fontFamily, fontSize, fontWeight, lineHeight, textTransform } = typeSettings.value as any;
    ...
    textStyle.lineHeight = cssToFigmaLineHeight(lineHeight) ?? textStyle.lineHeight;
    textStyle.textCase = cssToFigmaTextTransform(textTransform) ?? textStyle.textCase;
  }
}

/** @internal Find deleted tokens by comparing the old and new state */
function findDeletedTokens(oldState: TokenStoreModel, newState: TokenStoreModel): TokenStoreModel {
  ...
  for (const [type, tokens] of Object.entries(newState)) {
    ...
    missingTokenModel[type] = missingTokens.reduce((acc: DesignTokenMap, curr) => {
      acc[curr.name] = curr;
      return acc;
    }, {});
  }
 ...
 */
async function markDeletedPalettes(deletedPalettes: DesignTokenMap): Promise<void> {
  const paintStyles = figma.getLocalPaintStyles();
    ...
    paintStyle.paints = [
      {
        ...
        gradientTransform: [ // Rotation matrix
 ...
 */
async function markDeletedTypography(deletedTypographyTokens: DesignTokenMap): Promise<void> {
