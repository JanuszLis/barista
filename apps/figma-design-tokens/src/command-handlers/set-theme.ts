...
import { addThemeRelaunchDataToNode, isContainerNode, isGeometryNode } from '../shared/figma-util';
import { createWorker, fetchStyleMetadata } from '../shared/ipc';
import { setCachedRemoteStyles, getCachedRemoteStyles, getLastRemoteStyleFetchTime, setLastRemoteStyleFetchTime } from '../shared/storage';
...
export async function applyTheme(themeName: string): Promise<void> {
  ...
  const selectedFrames = figma.currentPage.selection
    .filter(isContainerNode) as FrameNode[];
  ...
  for (const node of selectedFrames) {
    await applyThemeToNode(themeName, node, paintStyles, styleIdReplacementCache);
...
async function applyThemeToNode(
  ...
  if (isGeometryNode(node)) {
    ...
    if (geometryNode.fillStyleId != figma.mixed) {
      ...
      geometryNode.fillStyleId = await getReplacementStyle(themeName, geometryNode.fillStyleId, paintStyles, styleIdReplacementCache);
    } else if (node.type === 'TEXT') {
      // The node is a text node with multiple text colors, so special handling is required
      await applyThemeToMixedColorText(themeName, geometryNode as TextNode, paintStyles, styleIdReplacementCache);
    }

    geometryNode.strokeStyleId = await getReplacementStyle(themeName, geometryNode.strokeStyleId, paintStyles, styleIdReplacementCache);
  }

  const containerNode = node as ChildrenMixin;
  if (containerNode.children) {
    for (const child of containerNode.children) {
      await applyThemeToNode(themeName, child, paintStyles, styleIdReplacementCache);
    }
  }
}
...
async function applyThemeToMixedColorText(
  ...
  for (let start = 0; start < stringLength - 1; start++) {
    ...
    for (let end = start + 1; end <= stringLength; end++) {
      ...
      const outerRangeFillStyleId = node.getRangeFillStyleId(start, Math.min(end + 1, stringLength));
      ...
      if (outerRangeFillStyleId === figma.mixed || end === stringLength) {
        ...
        if (rangeFillStyleId !== figma.mixed) {
          const newStyleId = await getReplacementStyle(themeName, rangeFillStyleId, paintStyles, styleIdReplacementCache);
...
async function getReplacementStyle(
  ...
  const currentStyle = paintStyles.find(style => currentStyleId.startsWith(style.id));
  if (!currentStyle) {
    return currentStyleId;
  }
  ...
  if (themeName && colorName) {
    if (ALL_THEMES.includes(themeName)) {
      const newStyleData = paintStyles.find(style => style.name === `${targetThemeName}/${colorName}`);
      let newStyle: PaintStyle | undefined;
      if (newStyleData?.remote) {
        newStyle = await figma.importStyleByKeyAsync(newStyleData.key) as PaintStyle;
      } else {
...
async function downloadRemoteStyles(): Promise<void> {
  ...
  if (!cachedRemoteStyles || (Date.now() - lastFetchDate.getTime()) > MIN_TIME_SINCE_LAST_FETCH) {
    try {
      createWorker();
      ...
      if (!cachedRemoteStyles) {
        alert('Initial download of remote styles failed. Please make sure that ' 
